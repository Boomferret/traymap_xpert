...
export const LayoutGrid = ({
  ...
  const [viewBox, setViewBox] = useState(() => {
    const canvasWidth = gridSize.width * cellSize || 1000;
    const canvasHeight = gridSize.height * cellSize || 1000;
    return {
      x: 0,
      y: 0,
      width: canvasWidth,
  ...
  // Update viewBox when grid size changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Calculate the actual canvas/image boundaries
    const canvasWidth = gridSize.width * cellSize;
    const canvasHeight = gridSize.height * cellSize;

    // Function to constrain viewBox to canvas boundaries
    const constrainViewBox = (newViewBox) => {
      // Minimum zoom constraints (don't zoom in too much)
      const minViewBoxWidth = Math.max(50, canvasWidth * 0.1);
      const minViewBoxHeight = Math.max(50, canvasHeight * 0.1);

      // Maximum zoom constraints (don't zoom out beyond actual canvas)
      const maxViewBoxWidth = canvasWidth;
      const maxViewBoxHeight = canvasHeight;

      // Constrain viewBox size
      const constrainedWidth = Math.max(minViewBoxWidth, Math.min(maxViewBoxWidth, newViewBox.width));
      const constrainedHeight = Math.max(minViewBoxHeight, Math.min(maxViewBoxHeight, newViewBox.height));

      // Constrain viewBox position to prevent showing white space
      // Don't allow any area outside the actual canvas (0,0 to canvasWidth,canvasHeight)
      // But allow small margins when zoomed out
      const margin = Math.max(0, (constrainedWidth - canvasWidth) / 2, (constrainedHeight - canvasHeight) / 2);
      const minX = -margin;
      const maxX = canvasWidth - constrainedWidth + margin;
      const minY = -margin;
      const maxY = canvasHeight - constrainedHeight + margin;

      let constrainedX, constrainedY;

      // If viewBox is same size or larger than canvas (zoomed out), center it exactly
      if (constrainedWidth >= canvasWidth) {
        constrainedX = (canvasWidth - constrainedWidth) / 2;
      } else {
        constrainedX = Math.max(minX, Math.min(maxX, newViewBox.x));
      }

      if (constrainedHeight >= canvasHeight) {
        constrainedY = (canvasHeight - constrainedHeight) / 2;
      } else {
        constrainedY = Math.max(minY, Math.min(maxY, newViewBox.y));
      }
    ...
    const handleWheel = (e) => {
      ...
      const constrainedViewBox = constrainViewBox({
        x: newX,
        y: newY,
        width: newWidth,
        height: newHeight,
    ...
    const handleMouseMove = (e) => {
      if (isPanning && panStart) {
        ...
        // Apply constraints and update state
        const constrainedViewBox = constrainViewBox(newViewBox);
        setViewBox(constrainedViewBox);
        setPanStart({ x: e.clientX, y: e.clientY });
      }
    };
    ...
    // Middle-click reset functionality
    let lastClickTime = 0;
    const handleMiddleClick = (e) => {
      ...
      if (timeSinceLastClick < 300) {
        // Reset view to show full canvas exactly
        const resetViewBox = {
          x: 0,
          y: 0,
          width: canvasWidth,
  ...
  }, [gridSize.width, gridSize.height, cellSize, viewBox, isPanning, panStart]);
      ...
      return (
        ...
        >
          ...
          <text
            ...
            y={-12}
            textAnchor="middle"
            className={`text-xs font-medium ${isSelected || isInheritTarget ? 'fill-blue-700' : 'fill-green-700'
              }`}
          >
            {name}
          </text>
      ...
      <div className="w-full h-full relative">
        ...
        >
          ...
          >
            ...
            {showInheritMenu && (
              <div
                className="absolute bg-white rounded-lg shadow-lg p-4 w-96 z-50 border"
                style={{
